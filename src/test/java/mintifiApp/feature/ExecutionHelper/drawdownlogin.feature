...
@generateid
Scenario: To verify 'Login with Cust id" with using otp
        Given url getUrl.mintifiBaseUrl + getUrl.typeAuthSendOtpAPI
        * headers getHeaders
        And request getRequestBodyLogin.verifyDrawdownLogin
        When method post
        Then status 200
        And print response
        ...
        * headers getHeaders
        * def requestBody = getRequestBodyLogin.verifyDDROtp
